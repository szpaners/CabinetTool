(function() {
  ...
  function updateHalfConfigUi(sectionRow, side) {
    ...
    const drawerFrontHeightGroup = sectionRow.querySelector(`[data-field="split_${side}_drawer_front_height_group"]`);
    const drawerFrontHeightInput = sectionRow.querySelector(`[data-field="split_${side}_drawer_front_height"]`);
    ...
    drawerFrontHeightGroup?.classList.toggle('is-hidden', !isDrawers);
    ...
    if (isDrawers) {
      if (drawerFrontHeightInput && Number(drawerFrontHeightInput.value || 0) < 1) drawerFrontHeightInput.value = 80;
  ...
  function updateSectionParamUi(sectionRow) {
    ...
    if (splitEnabledInput && splitLeftWidthGroup && splitLeftWidthInput && splitRightWidthGroup && splitRightWidthInput) {
      ...
      drawerFrontHeightGroup?.classList.toggle('is-hidden', splitEnabled || filling !== 'drawers');
  ...
  function readSectionFromRow(sectionRow, index) {
    ...
    const splitLeftDrawerFrontHeight = Math.max(Number(sectionRow.querySelector('[data-field="split_left_drawer_front_height"]').value || 80), 1);
    ...
    const splitRightDrawerFrontHeight = Math.max(Number(sectionRow.querySelector('[data-field="split_right_drawer_front_height"]').value || 80), 1);
    ...
    const splitLeftConfig = {
      ...
      drawer_front_height: splitLeftDrawerFrontHeight,
    ...
    const splitRightConfig = {
      ...
      drawer_front_height: splitRightDrawerFrontHeight,
  ...
  function addSection(section = {}) {
    ...
    row.querySelector('[data-field="split_left_drawer_front_height"]').value = section.params?.split_left?.drawer_front_height ?? section.params?.drawer_front_height ?? 80;
    ...
    row.querySelector('[data-field="split_right_drawer_front_height"]').value = section.params?.split_right?.drawer_front_height ?? section.params?.drawer_front_height ?? 80;
  ...
  function buildSaveParams() {
    ...
    return {
      cabinet_type: (typeof window.getActiveWardrobeType === 'function' ? window.getActiveWardrobeType() : 'wardrobe'),
      ...
      if (section.params.split_enabled) {
        ...
        const invalidSplit = splitSides.some((side) => {
          ...
          if (side.filling === 'drawers') {
            return Number(side.param || 0) < 1 || Number(side.drawer_front_height || 0) <= 0 || Number(side.drawer_width_reduction || 0) < 0 || Number(side.drawer_box_height_offset || 0) < 0;
          }
