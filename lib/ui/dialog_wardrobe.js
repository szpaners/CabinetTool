(function() {
  const SECTION_FILLINGS = {
    none: { hint: 'Brak parametru', min: 0, defaultValue: 0 },
    drawers: { hint: 'Liczba szuflad', min: 1, defaultValue: 3 },
    shelves: { hint: 'Liczba półek', min: 1, defaultValue: 4 },
    rod: { hint: 'Offset drążka od dołu sekcji (mm)', min: 0, defaultValue: 200 }
  };
  ...
  function readSectionFromRow(sectionRow, index) {
    ...
    if (filling === 'drawers') params.drawer_count = Math.max(Math.round(paramValue), 1);
  ...
  function updateRemainingHeight() {
    ...
    const usedHeight = sectionRows().reduce((sum, row) => sum + readSectionFromRow(row, 0).height, 0);
  ...
  function addSection(section = {}) {
    ...
    byId(form.sectionsList).appendChild(row);
    updateRemainingHeight();
  }

  function collectInteriorSections() {
    return sectionRows().map((row, index) => readSectionFromRow(row, index));
  }
  ...
  function validateBeforeSave() {
    ...
    const invalid = collectInteriorSections().find((section) => {
      ...
      if (section.filling === 'drawers') return Number(section.params.drawer_count || 0) < 1;
