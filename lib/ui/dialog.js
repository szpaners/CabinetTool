...
document.getElementById('width').value = params.width;
    ...
    const rawFrontType = (params.front_type || 'flat');
    const selectedFrontType = rawFrontType === 'rama' ? 'frame' : ['ryflowany', 'grooved', 'lamelowany'].includes(rawFrontType) ? 'lamella' : rawFrontType;
...
function updateCabinetPreview() {
    ...
    const frontType = document.getElementById('front_type').value;
    const frameWidth = Math.max(Number(document.getElementById('frame_width').value || 0), 0);
    const frameInnerThickness = Math.max(Number(document.getElementById('frame_inner_thickness').value || 0), 0);
    const grooveWidth = Math.max(Number(document.getElementById('groove_width').value || 0), 0);
    const grooveSpacing = Math.max(Number(document.getElementById('groove_spacing').value || 0), 0);
    const grooveDepth = Math.max(Number(document.getElementById('groove_depth').value || 0), 0);
    const frontThickness = Math.max(Number(document.getElementById('front_thickness').value || 0), 0);
    ...
    const panelThickness = Math.max(Number(document.getElementById('panel_thickness').value || 18), 0);
