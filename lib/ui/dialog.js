window.activeWardrobeType = 'wardrobe';

function setActiveWardrobeType(cabinetType) {
    window.activeWardrobeType = cabinetType === 'tall_unit' ? 'tall_unit' : 'wardrobe';
}

function getActiveWardrobeType() {
    return window.activeWardrobeType === 'tall_unit' ? 'tall_unit' : 'wardrobe';
}

function switchWardrobeType(element, cabinetType) {
    setActiveWardrobeType(cabinetType);
    switchTab(element, 'materials');
}

function switchTab(element, tabName) {
    console.log('switchTab called with:', element, tabName);
    if (tabName === 'materials') {
        const cabinetType = element?.dataset?.cabinetType;
        if (cabinetType) setActiveWardrobeType(cabinetType);
    }
...
function setFormData(params) {
    ...
    const isWardrobeFamily = ['wardrobe', 'tall_unit'].includes(params.cabinet_type);
    const targetTab = isWardrobeFamily ? 'materials' : 'dimensions';
    const targetButton = targetTab === 'materials'
        ? document.querySelector(`.tab[data-cabinet-type="${params.cabinet_type === 'tall_unit' ? 'tall_unit' : 'wardrobe'}"]`)
        : Array.from(document.querySelectorAll('.tab')).find(tab => tab.getAttribute('onclick')?.includes(`'${targetTab}'`));
    if (isWardrobeFamily) setActiveWardrobeType(params.cabinet_type);
...
function updateCabinetPreview() {
    ...
    const filling = isWardrobeTab ? 'none' : document.getElementById('filling').value;
    ...
    const cabinetWidth = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_width').value : document.getElementById('width').value) || 600), 1);
    const cabinetHeight = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_height').value : document.getElementById('height').value) || 800), 1);
    const cabinetDepth = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_depth').value : document.getElementById('depth').value) || 300), 1);
    const panelThickness = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_panel_thickness').value : document.getElementById('panel_thickness').value) || 18), 0);

    const blendLeftEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_left_checkbox').checked : document.getElementById('blend_left').checked;
    const blendRightEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_right_checkbox').checked : document.getElementById('blend_right').checked;
    const blendLeftValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_blend_left_value').value : document.getElementById('blend_left_value').value) || 0), 0);
    const blendRightValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_blend_right_value').value : document.getElementById('blend_right_value').value) || 0), 0);
    const blendLeftDepthEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_left_checkbox').checked : document.getElementById('blend_left_depth_checkbox').checked;
    const blendRightDepthEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_right_checkbox').checked : document.getElementById('blend_right_depth_checkbox').checked;
    const blendLeftDepthValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_blend_left_depth_value').value : document.getElementById('blend_left_depth_value').value) || 0), 0);
    const blendRightDepthValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_blend_right_depth_value').value : document.getElementById('blend_right_depth_value').value) || 0), 0);

    const cokolDolnyEnabled = isWardrobeTab ? document.getElementById('wardrobe_cokol_dolny_checkbox').checked : document.getElementById('cokol_dolny_checkbox').checked;
    const cokolGornyEnabled = isWardrobeTab ? document.getElementById('wardrobe_cokol_gorny_checkbox').checked : document.getElementById('cokol_gorny_checkbox').checked;
    const cokolDolnyValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_cokol_dolny_value').value : document.getElementById('cokol_dolny_value').value) || 0), 0);
    const cokolGornyValue = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_cokol_gorny_value').value : document.getElementById('cokol_gorny_value').value) || 0), 0);
