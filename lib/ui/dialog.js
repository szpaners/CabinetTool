...
function setFormData(params) {
    ...
    document.getElementById('corner_front_width').value = params.corner_front_width || params.corner_front_panel_width || 0;
    document.getElementById('corner_front_panel_side').value = ['left', 'right'].includes(params.corner_front_panel_side) ? params.corner_front_panel_side : 'left';
    ...
    document.getElementById('corner_front_checkbox').checked = !!params.front_enabled;
    document.getElementById('corner_front_technological_gap').value = params.front_technological_gap || 0;
    document.getElementById('corner_front_quantity').value = params.front_quantity || 1;
    document.getElementById('corner_front_opening_direction').value = params.front_opening_direction || 'prawo';
    document.getElementById('corner_front_handle').value = params.front_handle || 'J';
    document.getElementById('corner_front_type').value = selectedFrontType;
    document.getElementById('corner_frame_width').value = params.frame_width || 20;
    document.getElementById('corner_frame_inner_thickness').value = params.frame_inner_thickness || params.frame_inner_depth || 2;
    document.getElementById('corner_groove_width').value = params.groove_width || 12;
    document.getElementById('corner_groove_spacing').value = params.groove_spacing || 8;
    document.getElementById('corner_groove_depth').value = params.groove_depth || 3;
    ...
    document.getElementById('corner_blend_left_offset_value').value = params.blend_left_offset_value || 0;
    document.getElementById('corner_blend_right_offset_value').value = params.blend_right_offset_value || 0;
    ...
    toggleCornerFrontSection();
    toggleCornerFrontOpeningDirectionLock();
    updateCornerFrontType();
    ...
    updateCornerBlendOffsetVisibility();
...
function saveCabinet() {
    ...
    if (activeTab === 'corner') {
        ...
        const params = {
            ...
            connector_width: 0,
            corner_front_width: document.getElementById('corner_front_width').value,
            corner_front_panel_side: document.getElementById('corner_front_panel_side').value,
            ...
            front_enabled: document.getElementById('corner_front_checkbox').checked,
            front_technological_gap: document.getElementById('corner_front_checkbox').checked ? document.getElementById('corner_front_technological_gap').value : 0,
            front_quantity: document.getElementById('corner_front_quantity').value,
            front_type: document.getElementById('corner_front_type').value,
            front_handle: document.getElementById('corner_front_handle').value,
            frame_width: document.getElementById('corner_frame_width').value,
            frame_inner_thickness: document.getElementById('corner_frame_inner_thickness').value,
            groove_width: document.getElementById('corner_groove_width').value,
            groove_spacing: document.getElementById('corner_groove_spacing').value,
            groove_depth: document.getElementById('corner_groove_depth').value,
            front_opening_direction: document.getElementById('corner_front_opening_direction').value,
            ...
            blend_left_offset_value: document.getElementById('corner_blend_left_checkbox').checked ? document.getElementById('corner_blend_left_offset_value').value : 0,
            blend_right_offset_value: document.getElementById('corner_blend_right_checkbox').checked ? document.getElementById('corner_blend_right_offset_value').value : 0,
    ...
    const params = {
        ...
        blend_left_offset_value: 0,
        blend_right_offset_value: 0,
...
function toggleCornerFrontSection() {
    const enabled = document.getElementById('corner_front_checkbox').checked;
    document.getElementById('corner_front_details').classList.toggle('is-hidden', !enabled);

    updateCabinetPreview();
}

function toggleCornerFrontOpeningDirectionLock() {
    const frontQuantity = document.getElementById('corner_front_quantity');
    const openingDirection = document.getElementById('corner_front_opening_direction');
    openingDirection.disabled = Number(frontQuantity.value) === 2;

    updateCabinetPreview();
}

function updateCornerFrontType() {
    const frontType = document.getElementById('corner_front_type').value;
    document.getElementById('corner_frame_width_group').classList.toggle('is-hidden', frontType !== 'frame');
    document.getElementById('corner_frame_inner_thickness_group').classList.toggle('is-hidden', frontType !== 'frame');
    document.getElementById('corner_groove_width_group').classList.toggle('is-hidden', frontType !== 'lamella');
    document.getElementById('corner_groove_spacing_group').classList.toggle('is-hidden', frontType !== 'lamella');
    document.getElementById('corner_groove_depth_group').classList.toggle('is-hidden', frontType !== 'lamella');
...
function updateCornerBlendOffsetVisibility() {
    const panelSide = document.getElementById('corner_front_panel_side').value;
    ['left', 'right'].forEach(side => {
        const enabled = document.getElementById(`corner_blend_${side}_checkbox`).checked;
        const visible = enabled && panelSide === side;
        document.getElementById(`corner_blend_${side}_offset_group`).classList.toggle('is-hidden', !visible);
    });
}
...
function updateCabinetPreview() {
    const previewSvg = document.getElementById('preview_iso_svg');
    const summary = document.getElementById('preview_summary');

    if (!previewSvg || !summary) {
        return;
    }
    ...
    const frontEnabled = isWardrobeTab
        ...
        : isCornerTab
            ? document.getElementById('corner_front_checkbox').checked
        : (document.getElementById('front_checkbox').checked || filling === 'drawers');
    const frontQuantity = Number((isWardrobeTab ? document.getElementById('wardrobe_front_quantity').value : isCornerTab ? document.getElementById('corner_front_quantity').value : document.getElementById('front_quantity').value) || 1);
    const frontType = isWardrobeTab ? document.getElementById('wardrobe_front_type').value : isCornerTab ? document.getElementById('corner_front_type').value : document.getElementById('front_type').value;
    const frameWidth = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_frame_width').value : isCornerTab ? document.getElementById('corner_frame_width').value : document.getElementById('frame_width').value) || 0), 0);
    const frameInnerThickness = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_frame_inner_thickness').value : isCornerTab ? document.getElementById('corner_frame_inner_thickness').value : document.getElementById('frame_inner_thickness').value) || 0), 0);
    const grooveWidth = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_groove_width').value : isCornerTab ? document.getElementById('corner_groove_width').value : document.getElementById('groove_width').value) || 0), 0);
    const grooveSpacing = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_groove_spacing').value : isCornerTab ? document.getElementById('corner_groove_spacing').value : document.getElementById('groove_spacing').value) || 0), 0);
    const grooveDepth = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_groove_depth').value : isCornerTab ? document.getElementById('corner_groove_depth').value : document.getElementById('groove_depth').value) || 0), 0);
    const frontThickness = Math.max(Number((isWardrobeTab ? document.getElementById('wardrobe_front_thickness').value : isCornerTab ? document.getElementById('corner_front_thickness').value : document.getElementById('front_thickness').value) || 0), 0);
    const openingDirection = isWardrobeTab ? document.getElementById('wardrobe_front_opening_direction').value : isCornerTab ? document.getElementById('corner_front_opening_direction').value : document.getElementById('front_opening_direction').value;
    ...
    const blendLeftDepthEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_left_depth_checkbox').checked : isCornerTab ? document.getElementById('corner_blend_left_depth_checkbox').checked : document.getElementById('blend_left_depth_checkbox').checked;
    const blendRightDepthEnabled = isWardrobeTab ? document.getElementById('wardrobe_blend_right_depth_checkbox').checked : isCornerTab ? document.getElementById('corner_blend_right_depth_checkbox').checked : document.getElementById('blend_right_depth_checkbox').checked;
...
document.getElementById('corner_front_type').addEventListener('change', updateCornerFrontType);
document.getElementById('corner_front_quantity').addEventListener('change', toggleCornerFrontOpeningDirectionLock);
document.getElementById('corner_front_panel_side').addEventListener('change', updateCornerBlendOffsetVisibility);
